...
class TempSignUpBloc extends Bloc<TempSignUpEvent, TempSignUpState> {
  ...
  TempSignUpBloc({required UserSignUp userSignUp}) : _userSignUp = userSignUp,  super(TempSignUpInitial()) {
    on<TempSignUpEmailChangedEvent>((event,emit){
      if(!(EmailValidator.validate(event.email))){
        emit(TempSignUpInvalidState(errorMessage: "Please enter a valid email!"));
      } else {
    ...
    on<TempSignUpPasswordChangedEvent>((event,emit){
      if(event.password.length < 8){
        emit(TempSignUpInvalidState(errorMessage: "Please enter a valid password"));
    ...
    on<TempSignUpSubmittedEvent>((event,emit){
      if(EmailValidator.validate(event.email) && event.password.length > 8){
        ...
        _userSignUp.call(Parameters(email: event.email, password: event.password));
      } else {
        emit(TempSignUpInvalidState(errorMessage: "Email or password is not Valid"));
